...
def f_norm(pk, pk_simul):
    n = len(pk[0])
    pk_simul = pk_simul.view(-1, n)
    # pk = pk.view(-1, n)
    pk = pk.contiguous().view(-1, n)
    x = pk - pk_simul
    f_norm_error = torch.linalg.norm(x, ord='fro')

    return f_norm_error

def lr_decay_mine(optimizer, lr_now, gamma, epo):
    # if epo == 51:
    #     lr = lr_now / 10
    # # elif epo == 31:
    # #     lr = lr_now / 5
