...
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        smooth1 = smooth(p3d_h36[:, :, dim_used],
                         sample_len=opt.input_n + opt.output_n + opt.priv_n,
        ...
        p3d_sup_4 = p3d_h36.clone()[:, :, dim_used][:, -priv_n -out_n - in_n:].reshape(
            [-1, in_n + out_n + priv_n, len(dim_used) // 3, 3])
        p3d_sup_3 = smooth1.clone()[:, -priv_n -out_n - in_n:].reshape(
            [-1, in_n + out_n + priv_n, len(dim_used) // 3, 3])
        p3d_sup_2 = smooth2.clone()[:, -priv_n -out_n - in_n:].reshape(
            [-1, in_n + out_n + priv_n, len(dim_used) // 3, 3])
        p3d_sup_1 = smooth3.clone()[:, -priv_n -out_n - in_n:].reshape(
            [-1, in_n + out_n + priv_n, len(dim_used) // 3, 3])

        p3d_out_all_4, p3d_out_all_3, p3d_out_all_2, p3d_out_all_1 = net_pred(input, input_n=in_n, output_n=out_n, priv_n=priv_n, itera=itera)

        # 网络输出序列-用于计算mpjpe
        # [b, 35, 32, 3]
        # p3d_out_4 = p3d_h36.clone()[:, in_n:in_n + out_n + priv_n]
        ...
        # 检查输入10帧的差距
        # p3d_out_4 = p3d_h36.clone()[:, :in_n + out_n]
        # p3d_out_4[:, :, dim_used] = p3d_out_all_4[:, :in_n + out_n]
        # p3d_out_4[:, :, index_to_ignore] = p3d_out_4[:, :, index_to_equal]
        # p3d_out_4 = p3d_out_4.reshape([-1, in_n + out_n, 32, 3])
        p3d_h36 = p3d_h36.reshape([-1, in_n + out_n + priv_n, 32, 3])
